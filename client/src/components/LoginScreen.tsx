...
import { Shield, KeyRound, User, ChevronRight, AlertCircle, Sparkles, Server, Terminal, Lock } from "lucide-react";
        ...
        >
          ...
          <div className="flex flex-col gap-1 text-left border-b border-[#2C1F18] pb-4">
            ...
            <p className="text-[10.5px] text-[#A38F82] leading-relaxed">
              Enter your designated credentials. No account switching or claiming other members' seats is allowed in this enterprise node.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left font-sans">
            <div>
              <label className="text-[9px] font-mono font-bold text-[#A38F82] uppercase tracking-wider block mb-1.5">
                Developer Username ID (Permanent)
              </label>
          ...
          {/* Quick Demo Logins section - beautifully interactive user profiles */}
          {developers && developers.length > 0 && (
            <div className="flex flex-col gap-2.5 pt-5 border-t border-[#2C1F18] font-sans">
              <div className="flex items-center gap-2 justify-between">
                <span className="text-[9px] font-mono font-bold text-[#7A675C] uppercase tracking-wider flex items-center gap-1">
                  <Server className="h-3 w-3 text-teal-400" /> Active Roster Seats
                </span>
                <span className="text-[8px] font-mono text-teal-400 bg-teal-950/40 px-2 py-0.5 rounded border border-teal-900/40 font-bold uppercase">
